...
function SignUpSecond(props) {
  ...
  const handleInputPw = (event) => {
    ...
    const checkLength = event.target.value?.length > 7 && event.target.value?.length < 21;
    (checkLength) ?
      setIsPwLengthValidation(true) : setIsPwLengthValidation(false);
    (checkNum.test(event.target.value)) ?
      setCheckPwNumber(true) : setCheckPwNumber(false);
    (checkEng.test(event.target.value)) ?
      setCheckPwEnglish(true) : setCheckPwEnglish(false);
  }
  ...
  const API = "http://10.58.3.134:8000/user/signup";
      ...
      <SignUpWrapper>
        <Box>
          ...
          <Box2>
            ...
            <Text>
              로그인에 사용할<br/>{props.title}를 입력해주세요.
            </Text>
            {(props.title === "비밀번호") ? 
              props.data.map((el) =>
                <div key = {el.id}>
                  ...
                  <Input 
                    ...
                    onChange={
                      el.handleInput === "handleInputPw" ? handleInputPw : handleInputRePw
                    }
                  />
                  </InputId>
                  <Validation
                  id={userInputRePw ? isMatchPassword:checkPwEnglish && checkPwNumber && checkPwLength}>
            ...
            <NextButton
            id={ userInput ? id:checkPwEnglish && checkPwNumber && checkPwLength && isMatchPassword}
             type="button"
...
const Validation = styled.div`
  hieght: 40px;
  margin-bottom: 14px;
  color: ${props => props.userInputRePw ? (props.isMatchPassword ? "#375FFF":"#ff4800") : (props.checkPwEnglish && props.checkPwNumber && props.checkPwLength && props.isMatchPassword ? "#375FFF":"#c4c4c4")};
  font-size: 12px;
  texst-align: left;
`;
...
const NextButton = styled.button`
  ...
  height:48px;
  margin-top: 14px;
  background: ${props => props.userInputPw ? (props.checkPwEnglish && props.checkPwNumber && props.checkPwLength && props.isMatchPassword ? "#375FFF":"#c4c4c4") : (props.id ? "#375FFF":"#c4c4c4")};
  font-size: 14px;
  color: #fff;
  outline: none;
  border: none;
